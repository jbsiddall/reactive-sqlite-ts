/**
 * The `libsqlite3` symbol table and the one `dlopen` that binds it.
 *
 ...
 * to the surface this library uses, and — the one behavioural change — the
 * library is never downloaded. Upstream fell back to fetching a prebuilt
 * `libsqlite3` over the network; here the only two sources are
 * `DENO_SQLITE_PATH` and the vendored build, so the file this process dlopens
 * is always one the operator can name.
 *
 * That matters beyond supply chain: `src/hooks.ts` borrows the `sqlite3*` this
 * module opened and calls SQLite through its own `Deno.dlopen` of the SAME
 * path. Two different builds behind one handle is a SIGSEGV, so the path has
 * to be knowable, and a silent download is not.
 *
 * @module
 */
...
/** `Deno.env.get` without requiring the permission — unset reads as absent. */
...
/**
 * The library this process opened, and how it was chosen.
 *
 * Exported because a caller that has to dlopen the same file — which is every
 * caller of `withEvents` — should be able to ask rather than re-derive it.
 */
export const libraryPath: string = tryGetEnv("DENO_SQLITE_PATH") ??
  vendoredLibraryPath();

const lib = ((): Deno.DynamicLibrary<typeof symbols>["symbols"] => {
  try {
    return Deno.dlopen(libraryPath, symbols).symbols;
  } catch (e) {
    if (e instanceof Deno.errors.PermissionDenied) throw e;
    throw new Error(`Failed to load ${libraryPath} as libsqlite3`, {
      cause: e,
    });
  }
})();
